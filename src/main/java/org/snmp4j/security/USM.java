...
public class USM extends SNMPv3SecurityModel {
   ...
   * The security protocols instance defined by {@link SecurityProtocols#getInstance()} with the
  ...
  /**
   * Looks up a {@link UsmUserEntry} by an engine ID and
   ...
   * {@link UsmUserEntry} is returned with just the provided
   * securityName set.
   * @param engineID
   *    an engine ID.
   * @param securityName
   *    a security name.
   * @return
   *    an localized {@link UsmUserEntry} if the provided
  ...
  /**
   * Removes all USM user from the internal user name table with the specified user
   * name. This is the same as {@link #removeAllUsers(OctetString, OctetString)}
