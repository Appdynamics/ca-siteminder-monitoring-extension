...
public class MetricValueTransformerTest {
    ...
    public void whenMatchingConversionValuesAndMultiplier_thenConvert(){
        ...
        conversionValues.put("Active",1);
        props.setConversionValues(conversionValues);
        props.setMultiplier(2);
        BigDecimal retValue = mvc.transform("policyServerStatus","Active",props);
    ...
    @Test
    public void whenDeltaIfNoCached_thenReturnNull(){
        MetricProperties props = new MetricProperties();
        props.setDelta(true);
        BigDecimal retValue = mvc.transform("policyServerStatus","3.4",props);
        Assert.assertTrue(retValue == null);
    }

    @Test
    public void whenDeltaIfCached_thenReturnDifference(){
        MetricProperties props = new MetricProperties();
        props.setDelta(true);
        BigDecimal retValue = mvc.transform("policyServerStatus","3.4",props);
        BigDecimal retValue1 = mvc.transform("policyServerStatus","4.5",props);
        Assert.assertTrue(retValue1.equals(BigDecimal.valueOf(1.1d)));
    }
}
